...
除了會尋找 CSRF token 作為「POST」參數，中介層也會檢查 `X-XSRF-TOKEN` 請求標頭。 You could, for example, store the token in a "meta" tag and instruct jQuery to add it to all request headers:
...
如果你想要讓路由參數總是被一個給定的正規表達式約束，您可以使用 `pattern` 方法。你應該在 `RouteServiceProvider` 的 `boot` 方法定義這些模式：
