...
如你所見，`view` 輔助方法的第一個參數是 `greeting`，`view` 輔助方法會用這個參數幫你對應到 `resources/views` 目錄內的 `greeting.blade.php` 或是 `greeting.php` 視圖檔案；傳遞到 `view` 輔助方法的第二個參數是一個陣列 `['name' => 'James']`，代表你希望視圖檔案使用這個資料來改變呈現的內容。
